...
import training.training_loop_partial
...
config_presets = {
    ...
    # 224 and RGB
    'gene-img224-xs':   dnnlib.EasyDict(duration=2048<<20, batch=2048, channels=128, lr=0.0120, decay=70000, dropout=0.00, P_mean=-0.4, P_std=1.0),
    'gene-img224-s':    dnnlib.EasyDict(duration=2048<<20, batch=2048, channels=192, lr=0.0100, decay=70000, dropout=0.00, P_mean=-0.4, P_std=1.0),
    'gene-img224-m':    dnnlib.EasyDict(duration=2048<<20, batch=2048, channels=256, lr=0.0090, decay=70000, dropout=0.10, P_mean=-0.4, P_std=1.0),
}
...
def setup_training_config(preset='gene-img224-xs', **opts):
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.GenePatchDataset', patch_path=opts.patch_path, gene_path=opts.gene_path)
    c.valid_dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.GenePatchDataset', patch_path=opts.valid_patch_path, gene_path=opts.valid_gene_path)
    c.valid_batch_size = opts.get('valid_batch_size', 0)
    ...
    # Hyperparameters.
    c.update(total_nimg=opts.duration, batch_size=opts.batch_size)
    c.network_kwargs = dnnlib.EasyDict(class_name='training.networks_edm2.Precond', 
                                       model_channels=opts.channels,
                                       dropout=opts.dropout,
                                       no_guidance_prob=opts.p,
                                       embed_dim=opts.embed_dim,
                                       num_gene_blocks=opts.num_gene_blocks)
    ...
    c.batch_gpu = opts.get('batch_gpu', 0) or None  # random set
...
def print_training_config(run_dir, c):
    ...
    dist.print0(f'Patch Dataset path:      {c.dataset_kwargs.patch_path}')
    dist.print0(f'Gene Dataset path:       {c.dataset_kwargs.gene_path}')
    dist.print0(f'Valid Patch Dataset path:       {c.valid_dataset_kwargs.patch_path}')
    dist.print0(f'Valid Gene Dataset path:       {c.valid_dataset_kwargs.gene_path}')
    dist.print0(f'Gene Embedding dim:       {c.network_kwargs.embed_dim}')
    dist.print0(f'Number of Gene Encoder Blocks:       {c.network_kwargs.num_gene_blocks}')
    
    dist.print0(f'Number of GPUs:          {dist.get_world_size()}')
    dist.print0(f'Batch size:              {c.batch_size}')  # 2048
...
# @click.option('--patch_path',       help='Path to the patch dataset', metavar='ZIP|DIR|h5',        type=str, required=True)
# @click.option('--gene_path',        help='Path to the gene dataset', metavar='ZIP|DIR|h5',         type=str, required=True)
# @click.option('--valid_patch_path',       help='Path to the validation patch dataset', metavar='ZIP|DIR|h5',        type=str, required=True)
# @click.option('--valid_gene_path',        help='Path to the validation gene dataset', metavar='ZIP|DIR|h5',         type=str, required=True)
@click.option('--patch_path', 
              help='Paths to the patch datasets (can pass multiple paths)', 
              metavar='ZIP|DIR|h5', 
              type=str, 
              multiple=True, 
              required=True)
@click.option('--gene_path', 
              help='Paths to the gene datasets (can pass multiple paths)', 
              metavar='ZIP|DIR|h5', 
              type=str, 
              multiple=True, 
              required=True)
@click.option('--valid_patch_path', 
              help='Paths to the validation patch datasets (can pass multiple paths)', 
              metavar='ZIP|DIR|h5', 
              type=str, 
              multiple=True, 
              required=True)
@click.option('--valid_gene_path', 
              help='Paths to the validation gene datasets (can pass multiple paths)', 
              metavar='ZIP|DIR|h5', 
              type=str, 
              multiple=True, 
              required=True)
@click.option('--valid_batch_size',       help='Total batch size', metavar='NIMG',              type=parse_nimg, default=None)
@click.option('--valid_interval_nimg',       help='Interval of validation', metavar='NIMG',     type=parse_nimg, default='128Mi', show_default=True)
...
@click.option('--p',                help='Hyperparameter p',                                    type=float, required=True)
# Gene Encoder
@click.option('--embed_dim',        help='Gene embedding dim',                                  type=int, required=True)
@click.option('--num_gene_blocks',  type=int, required=True, help='Number of gene processing blocks.')

# Hyperparameters.
@click.option('--duration',         help='Training duration', metavar='NIMG',                   type=parse_nimg, default=None)
@click.option('--batch_size',       help='Total batch size', metavar='NIMG',                    type=parse_nimg, default=None)
